...
### `eval/tool-calling-bench/benchmark.py`
Single-file benchmark comparing three Anthropic API tool-calling strategies (naive sequential, parallel sequential, code mode) on a 20-city weather lookup. Measures inference calls, tool executions, latency, and tokens.

### `eval/tool-calling-bench/README.md`
How to run the tool-calling benchmark: dependencies, env vars, expected runtime, sandbox caveats.

### `eval/tool-calling-bench/results.json`
Generated by `benchmark.py`: structured per-approach metrics (median/min/max latency, mean inference calls and tokens) plus per-run breakdown.

### `eval/tool-calling-bench/SUMMARY.md`
Generated by `benchmark.py`: human-readable comparison table and honest commentary on the gap between approaches.
